...
try {
    ...
    $conex = $con->conectar();
    $stmt = $conex->prepare("SELECT IdSafeLaunch, TIME(Fecha) as Hora, Nomina, NumeroParte, Estacion, CodigoDefecto, Estado FROM Safe_Launch WHERE DATE(Fecha) = ? AND Linea = ? AND Estado = 0 ORDER BY Fecha DESC");
